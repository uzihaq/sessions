...
const prettydDir = join(repoRoot, 'prettyd');
const goDir = join(repoRoot, 'prettygo');
...
function daemonLayout(name) {
  ...
  const stateRoot = join(home, '.local', 'state', 'pretty-PTY');
  return {
    root,
    home,
    stateRoot,
...
async function prepareLayout(layout) {
  ...
  // explicitly configured PRETTYD_STATE_DIR whose basename is not "runners".
...
function daemonEnv(layout, port, extra = {}) {
  return {
    ...
    PRETTYD_PORT: String(port),
    PRETTYD_STATE_DIR: layout.runnerState,
...
async function requestSnapshot(base, id) {
  ...
  return {
    ...
    signature: {
      status: response.status,
      contentType: response.headers.get('content-type'),
      xPrettySeq: shape(response.headers.get('x-pretty-seq')),
      body: shape(text)
    }
  };
}
...
function loadWebSocket() {
  const wsModule = require(join(prettydDir, 'node_modules', 'ws'));
...
async function frontendSmoke(base) {
  ...
  try {
    ...
    await page.evaluateOnNewDocument(() => {
      window.localStorage.setItem('pretty-pty:viewMode', 'terminal');
...
async function main() {
  ...
  await Promise.all([
    runLogged('build-go-prettyd', 'go', ['build', '-o', join(binDir, 'prettyd'), './cmd/prettyd'], {
      cwd: goDir,
      env: {
        ...process.env,
        HOME: buildHome,
    ...
    runLogged('build-go-runner', 'go', ['build', '-o', join(binDir, 'runner'), './cmd/runner'], {
      cwd: goDir,
      env: {
        ...process.env,
        HOME: buildHome,
    ...
    runLogged('build-ts-prettyd', join(prettydDir, 'node_modules', '.bin', 'tsc'), [
      '-p', join(prettydDir, 'tsconfig.json'), '--outDir', tsRuntime
  ...
  await symlink(join(prettydDir, 'node_modules'), join(tsRuntime, 'node_modules'), 'dir');
  ...
  try {
    ...
    goDaemon = startLogged('go-daemon', join(binDir, 'prettyd'), [], {
      cwd: repoRoot,
      env: daemonEnv(goLayout, goPort, {
        PRETTYD_RUNNER: join(binDir, 'runner'),
        PRETTYD_WEB_DIR: frontendDist
      })
    });
