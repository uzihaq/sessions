...
commands=(pretty prettyd runner)
...
if [[ "$dry_run" == true ]]; then
  ...
  for target in "${targets[@]}"; do
    goos="${target%/*}"
    goarch="${target#*/}"
    archive="pretty-pty_${version}_${goos}_${goarch}.tar.gz"
...
scratch_dir="$(mktemp -d "${TMPDIR:-/tmp}/pretty-pty-release.XXXXXX")"
cleanup() {
  rm -rf "$scratch_dir"
}
trap cleanup EXIT
...
for target in "${targets[@]}"; do
  ...
  archive_name="pretty-pty_${version}_${goos}_${goarch}.tar.gz"
