...
PORT=8898
RUNNER_BIN=/tmp/prettygo-runner-interop
...
# These are fixed disposable paths, never the user's default Pretty state.
rm -rf "$STATE" "$SCRATCH_HOME" "$WORK"
mkdir -p "$STATE" "$SCRATCH_HOME" "$WORK"

echo '$ CGO_ENABLED=0 /opt/homebrew/bin/go build -o /tmp/prettygo-runner-interop ./cmd/runner'
(
  cd "$ROOT/prettygo"
  CGO_ENABLED=0 "$GO" build -o "$RUNNER_BIN" ./cmd/runner
)
...
echo '$ HOME=/tmp/gorunner-home PRETTYD_STATE_DIR=/tmp/gorunner-state PRETTYD_PORT=8898 node prettyd/dist/server.js'
env -i \
  HOME="$SCRATCH_HOME" \
  PATH=/opt/homebrew/bin:/usr/bin:/bin \
  LANG=en_US.UTF-8 \
  ...
  PRETTYD_PORT="$PORT" \
  /opt/homebrew/bin/node "$ROOT/prettyd/dist/server.js" >"$DAEMON_OUT" 2>&1 &
daemon_pid=$!
...
MODULE="$ROOT/prettyd/dist/persistentLog.js" EVENTS="$STATE/$id.events" MARKER="$marker" \
