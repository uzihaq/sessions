...
func TestWorktreeCreateUsesNameBaseProvenanceAndCollisionSuffix(t *testing.T) {
	...
	if first.Cwd != wantPath || first.WorktreePath != wantPath || first.Branch != "pretty/fix-parser" ||
	...
	for key, want := range map[string]string{
		"worktree_path": wantPath, "branch": "pretty/fix-parser", "base": "main", "source_repo": repo,
	} {
		if payload[key] != want {
	...
	if override.Base != "release" || override.Branch != "pretty/release-fix" {
	...
	if collision.Cwd == first.Cwd || collision.Branch == first.Branch ||
		!strings.HasPrefix(collision.Branch, "pretty/fix-parser-") || !strings.HasPrefix(filepath.Base(collision.Cwd), "fix-parser-") {
...
func TestWorktreeCreateRollsBackIfWriteAheadLedgerBoundaryFails(t *testing.T) {
	...
	if _, err := gitOutput(context.Background(), repo, "show-ref", "--verify", "--quiet", "refs/heads/pretty/rollback-me"); err == nil {
...
func initWorktreeTestRepo(t *testing.T, root, name string) string {
	...
	gitTest(t, repo, "config", "user.name", "Pretty Test")
	gitTest(t, repo, "config", "user.email", "pretty@example.test")
