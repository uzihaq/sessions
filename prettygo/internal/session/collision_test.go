...
func TestConversationIdentityCollisionRefuseForceFreshAndAfterKill(t *testing.T) {
	...
	want := fmt.Sprintf("conversation %s is already live as \"primary\" (session %s) — attach with `pretty attach %s`, or re-run with --force to take over.", refuseProvider, primary.ID, primary.ID)
