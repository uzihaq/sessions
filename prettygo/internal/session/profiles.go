...
func (m *Manager) prepareProfile(command, name string) (string, error) {
	...
	if m.config.UserStateRoot == "" {
		return "", errors.New("--profile requires a configured Pretty user state root")
	}
...
func (m *Manager) Profiles(ctx context.Context) ([]ProfileStatus, error) {
	if m.config.UserStateRoot == "" {
		return nil, errors.New("profile listing requires a configured Pretty user state root")
	}
