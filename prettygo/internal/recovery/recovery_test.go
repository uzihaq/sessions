...
func scratchConfig(root string) state.Config {
	return state.Config{
		...
		StateRoot: filepath.Join(root, "pretty-state"), UserStateRoot: filepath.Join(root, "pretty-user-state"),
		RunnerStateDir: filepath.Join(root, "pretty-state", "runners"),
		TokenPath:      filepath.Join(root, "pretty-state", "token"), OpenPath: filepath.Join(root, "pretty-state", "open"),
