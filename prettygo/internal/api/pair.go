...
const pairTicketGoneMessage = "Pairing ticket is invalid, expired, or already used. Run `pretty pair` to create a new one."
...
func (s *Server) handlePairClaimRoute(response http.ResponseWriter, request *http.Request, corsOrigin string) bool {
	...
	if errors.Is(err, errPairRateLimit) {
		s.sendJSON(response, http.StatusTooManyRequests, map[string]any{
			"error": "Too many failed pairing attempts. Wait one minute, then run `pretty pair` again.",
		}, corsOrigin)
		return true
	}
