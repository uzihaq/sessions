...
func (l *lanListener) enable(persist bool) (LANState, error) {
	...
	if err != nil {
		return l.stateLocked(), fmt.Errorf("could not open the LAN listener at %s: %w; make sure this Mac is still on the network that owns %s and port %d is free, then retry `pretty lan enable`", address, err, host, l.config.Port)
	}
	...
	go func() {
		if err := server.Serve(listener); err != nil && !errors.Is(err, http.ErrServerClosed) && !errors.Is(err, net.ErrClosed) {
			log.Printf("prettyd: LAN listener error: %v", err)
			l.mu.Lock()
			if l.server == server {
				l.server = nil
				l.host = ""
...
func (s *Server) RestoreLAN(logf func(string, ...any)) {
	...
	if err != nil {
		logf("prettyd: could not load LAN setting: %v; continuing without LAN access", err)
		return
	}
	if !settings.LAN {
		return
	...
	if err != nil {
		logf("prettyd: LAN access is enabled in settings but could not start: %v; continuing without LAN access", err)
		return
	}
	logf("prettyd: LAN access listening on %s", *current.URL)
}
