...
func DefaultPath() (string, error) {
	...
	return filepath.Join(home, "Library", "Application Support", "pretty-PTY", "ledger", "lanes.sqlite3"), nil
}

// ResolvePath applies PRETTY_LEDGER_PATH unless Options.Path is explicit.
func ResolvePath(explicit string) (string, error) {
	path := explicit
	if path == "" {
		path = os.Getenv("PRETTY_LEDGER_PATH")
	}
	if path == "" {
		var err error
