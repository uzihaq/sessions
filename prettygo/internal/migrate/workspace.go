...
func PrepareWorkspace(ctx context.Context, cwd, sourceID string, options WorkspaceOptions) (Workspace, error) {
	...
	checkpoint := fmt.Sprintf("refs/pretty/checkpoints/%s-%d", part, now().UnixMilli())
...
func snapshotWorktree(ctx context.Context, root, head, sourceID string) (string, error) {
	index, err := os.CreateTemp("", "pretty-move-index-")
	if err != nil {
	...
	environment := append(os.Environ(),
		...
		"GIT_AUTHOR_NAME=pretty move", "GIT_AUTHOR_EMAIL=pretty-move@localhost",
		"GIT_COMMITTER_NAME=pretty move", "GIT_COMMITTER_EMAIL=pretty-move@localhost",
	)
	...
	commit, err := gitOutput(ctx, root, environment, "commit-tree", tree, "-p", head, "-m", "pretty move checkpoint for "+sourceID)
	if err != nil {
