...
func ledgerTestConfig(t *testing.T) Config {
	t.Helper()
	root := t.TempDir()
	return Config{
...
func TestLedgerWriteAheadBoundariesAreCommittedBeforeLaunchAndKill(t *testing.T) {
	...
	registry := NewRegistryWithLedger(config, launcher, store.Boundaries(), store.Observations())
	...
	if err := session.RequestKill(context.Background()); err != nil {
		t.Fatal(err)
	}
...
func TestLedgerBoundaryErrorsAbortTheirSideEffects(t *testing.T) {
	t.Run("created failure aborts launch", func(t *testing.T) {
		...
		registry := NewRegistryWithLedger(config, launcher, failingBoundary{createErr: errors.New("disk full")}, nil)
		_, err := registry.Create(context.Background(), CreateSessionRequest{Cmd: "/bin/sh", Cwd: config.DefaultCwd})
	...
	t.Run("tombstone failure aborts kill", func(t *testing.T) {
		...
		registry := NewRegistryWithLedger(config, launcher, boundary, nil)
		created, err := registry.Create(context.Background(), CreateSessionRequest{Cmd: "/bin/sh", Cwd: config.DefaultCwd})
		if err != nil {
			t.Fatal(err)
		}
		session, _ := registry.Get(created.ID)
		if err := session.RequestKill(context.Background()); err == nil || !strings.Contains(err.Error(), "before runner kill") {
			t.Fatalf("kill err=%v", err)
		}
		if !session.Input(context.Background(), "still alive") {
