...
This contract is implemented by `prettyd/src/runnerProtocol.ts`,
`prettyd/src/runner.ts`, `prettyd/src/runnerClient.ts`, and the daemon-side
registration logic in `prettyd/src/sessions.ts`.

## Transport and socket ownership

Each runner owns one Unix domain stream socket named `<id>.sock` in the runner
state directory. It accepts multiple clients, although prettyd is normally the
...
For both a newly created runner and startup discovery, prettyd:
