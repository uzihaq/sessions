# prettyd HTTP API contract

This document records the behavior of the normative TypeScript implementation,
principally `prettyd/src/http.ts`. It describes observed compatibility behavior,
including quirks; it is not a redesign.

## Listener and common behavior

- The default listener is `127.0.0.1:8787`. `PRETTYD_HOST` and
  `PRETTYD_PORT` override it. The server refuses `0.0.0.0`, `::`, `::0`, and
...
64 lowercase hex characters stored in `~/.local/state/pretty-PTY/token`; it is
created lazily on the first protected request if no valid token exists. A
present `~/.local/state/pretty-PTY/open` file bypasses token auth. Failed auth is
...
1. its serialized origin is exactly `https://pretty-pty.somewhere.tech` or
...
xterm buffer as the body, and `X-Pretty-Seq: <decimal sequence>`. If an allowed
Origin was present it also sets that ACAO value and
`Access-Control-Expose-Headers: X-Pretty-Seq`. The success path does not set
...
`X-Pretty-Filename` defaults to `file`; `Content-Type` is accepted but not used
...
The destination is the fixed `~/.local/state/pretty-PTY/uploads/` directory,
not the runner `PRETTYD_STATE_DIR`. Responses:
...
```json
{"path":"/absolute/path","label":"~/pretty/path","kind":"home"}
```
...
The web root is `PRETTYD_WEB_DIR` when set; otherwise it prefers
