...
func (a *app) cmdDevices(args []string) error {
	...
	return fail(1, "usage: pretty devices [revoke <id-or-prefix>]")
}
...
func (a *app) revokePairedDevice(prefix string) error {
	...
	if len(matches) == 0 {
		return fail(1, "no paired device matches %q; run `pretty devices` to list device ids", prefix)
	}
	if len(matches) > 1 {
		return fail(1, "device prefix %q is ambiguous (%d matches); use more characters from `pretty devices`", prefix, len(matches))
	}
	matched := matches[0]
...
func (a *app) fetchPairedDevices() ([]pairedDevice, error) {
	...
	if err := json.Unmarshal(response.body, &listed); err != nil {
		return nil, fail(2, "prettyd returned an invalid device list: %s", err)
	}
