...
// Run explicitly with:
//
//	CXWIRE_INTEGRATION=1 CGO_ENABLED=0 go test -v ./cmd/pretty -run '^TestCXWireRealTurnRestartReattach$' -count=1
func TestCXWireRealTurnRestartReattach(t *testing.T) {
	...
	root, err := os.MkdirTemp("/tmp", "pretty-cxwire-")
	if err != nil {
		t.Fatal(err)
	}
	...
	t.Setenv("PRETTY_CODEX_APPSERVER", "1")
	...
	t.Setenv("PRETTY_CODEX_APP_SERVER_SOCKET", appSocket)

	runnerBinary := filepath.Join(root, "runner")
	moduleRoot := filepath.Clean(filepath.Join(mustGetwd(t), "..", ".."))
	build := exec.CommandContext(ctx, "go", "build", "-o", runnerBinary, "./cmd/runner")
	...
	if err := json.Unmarshal(createdOutput, &created); err != nil {
		t.Fatalf("decode pretty new: %v\n%s", err, createdOutput)
	}
	if created.Kind != state.KindCodexAppServer || created.ConversationID == "" {
		t.Fatalf("pretty new metadata = %#v", created)
	}
	...
	for time.Now().Before(deadline) {
		...
		case result := <-sendDone:
			if result.code != 0 {
				t.Fatalf("pretty send: exit=%d stderr=%q stdout=%q", result.code, result.stderr, result.output)
			}
	...
	if sendDone != nil {
		...
		case <-time.After(5 * time.Second):
			t.Fatal("pretty send did not return after the structured user event")
		}
	}
	current, ok := manager.Get(created.ID)
...
func runCXWireCLI(t *testing.T, serverURL string, args ...string) []byte {
	...
	if code := run(arguments, strings.NewReader(""), &stdout, &stderr); code != 0 {
		t.Fatalf("pretty %s: exit=%d stderr=%q stdout=%q", strings.Join(args, " "), code, stderr.String(), stdout.String())
	}
...
func assertCXWireLast(t *testing.T, encoded []byte, want string) {
	...
	if err := json.Unmarshal(encoded, &turns); err != nil {
		t.Fatalf("decode pretty last: %v\n%s", err, encoded)
	}
	if len(turns) != 1 || strings.TrimSpace(turns[0].Text) != want {
		t.Fatalf("pretty last = %#v, want assistant %q", turns, want)
	}
}
