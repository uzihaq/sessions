...
// Run explicitly with:
//
//	CLAUDEP_INTEGRATION=1 CGO_ENABLED=0 go test -v ./cmd/pretty -run '^TestClaudePRealTurnsRestartReattach$' -count=1
func TestClaudePRealTurnsRestartReattach(t *testing.T) {
	...
	root, err := os.MkdirTemp("/tmp", "pretty-claudep-")
	if err != nil {
		t.Fatal(err)
	}
...
func sendClaudePTurn(t *testing.T, serverURL string, manager *sessionruntime.Manager, id, prompt, expected string, assistantCount int) claudePTurnObservation {
	...
	if code != 0 {
		t.Fatalf("pretty send: exit=%d stderr=%q stdout=%q", code, stderr.String(), stdout.String())
	}
	sawWorking := false
