...
func (a *app) cmdMove(args []string) error {
	const usage = "usage: pretty move <session> --to <target-endpoint> [--token T] [--dry-run] [--allow-dirty]"
	...
	_, err = fmt.Fprintf(a.stdout, "source still live; kill with pretty kill %s once verified\n", source.ID)
	return err
}
