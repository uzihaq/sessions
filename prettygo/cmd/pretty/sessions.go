...
func parseLSListOptions(args []string) (lsListOptions, error) {
	...
	for _, arg := range args {
		...
		default:
			return options, fail(1, "usage: pretty ls [--mine | --all] [-a | --include-exited]")
		}
	}
...
func parseSessionsListOptions(args []string) (sessionsListOptions, error) {
	...
	for index := 0; index < len(args); index++ {
		...
		case "--owner":
			...
			index++
		default:
			return options, fail(1, "usage: pretty sessions [--mine | --owner ID | --all] [--include-closed]")
		}
	}
	selectors := 0
	if options.mine {
...
func (a *app) resolveOwnershipScope(explicitOwner, explicitSession string) (ownershipScope, error) {
	...
	if ownerEnvironment != "" && strings.TrimSpace(ownerEnvironment) != ownerEnvironment {
		return ownershipScope{}, fail(1, "PRETTY_OWNER_ID must not contain surrounding whitespace")
	}
	if ownerEnvironment != "" {
	...
	if sessionEnvironment != "" && strings.TrimSpace(sessionEnvironment) != sessionEnvironment {
		return ownershipScope{}, fail(1, "PRETTY_SESSION_ID must not contain surrounding whitespace")
	}
	if sessionEnvironment != "" {
		if !looksLikeLaneID(sessionEnvironment) {
			return ownershipScope{}, fail(1, "PRETTY_SESSION_ID is not a session UUID")
		}
...
func writeOSUserScope(writer io.Writer, scope ownershipScope) {
	_, _ = fmt.Fprintf(writer, "ownership scope: OS user %s (no PRETTY_OWNER_ID or PRETTY_SESSION_ID)\n", scope.id)
}
