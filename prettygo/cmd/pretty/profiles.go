...
func (a *app) cmdProfiles(args []string) error {
	if len(args) != 0 {
		return fail(1, "usage: pretty profiles (Pretty never deletes profile credentials; review the path from `pretty profiles` and remove it manually)")
	}
	var response struct {
	...
	_, err := fmt.Fprintln(a.stdout, "Pretty never deletes profile credentials; remove one manually only after reviewing its PATH above.")
	return err
}
