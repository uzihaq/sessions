...
func (a *app) cmdPair(args []string) error {
	...
	if len(args) != 0 {
		return fail(1, "usage: pretty pair [--name NAME]")
	}
	...
	if ticket.Ticket == "" || ticket.ExpiresAt.IsZero() {
		return fail(2, "prettyd returned an invalid pairing ticket; retry `pretty pair`, and check the daemon log if it still fails")
	}
	...
	if endpoint == "" {
		return fail(2, "no verified phone-reachable endpoint is enabled. Enable one first: `pretty lan enable` (same WiFi) or `pretty remote enable` (anywhere), then retry `pretty pair`.")
	}

	pairURL, err := pairingURL(endpoint, ticket.Ticket)
	if err != nil {
		return fail(2, "prettyd reported an invalid pairing endpoint %q: %s", endpoint, err)
	}
	if a.wantJSON {
