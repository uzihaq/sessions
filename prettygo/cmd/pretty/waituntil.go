...
const waitUntilUsage = "usage: pretty wait <id> --until commit [--timeout 30s] | pretty wait <id> --until-file-contains FILE STRING [--timeout 30s] | pretty wait <id> --until-idle-stable DUR [--timeout 30s] [--any]"
...
func selectWaitTarget(idOrPrefix string, candidates []waitTarget) (waitTarget, error) {
	...
	return waitTarget{}, fail(1, "ambiguous session prefix '%s' — run `pretty ls`", idOrPrefix)
}

func (a *app) runnerMetadataTargets() ([]waitTarget, error) {
	dir := os.Getenv("PRETTYD_STATE_DIR")
	if dir == "" {
		dir = filepath.Join(a.home, ".local", "state", "pretty-PTY", "runners")
	}
