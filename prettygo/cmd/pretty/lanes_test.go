...
func TestRunPostsHeadlessLaneRequest(t *testing.T) {
	...
	t.Setenv("PRETTY_SESSION_ID", creator)
	...
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		...
		creatorHeader = request.Header.Get("X-Pretty-Creator-Session")
...
func TestRunExplicitOwnerRequiresDetachAndForwardsExternalRoot(t *testing.T) {
	...
	t.Setenv("PRETTY_SESSION_ID", inherited)
	t.Setenv("PRETTY_OWNER_ID", "")
	root := t.TempDir()
	...
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		requests++
		ownerHeader = request.Header.Get("X-Pretty-Owner-ID")
		sessionHeader = request.Header.Get("X-Pretty-Creator-Session")
		response.Header().Set("Content-Type", "application/json")
		response.WriteHeader(http.StatusCreated)
...
func TestLaneMineResolutionPrecedenceSubtreeDirectAndOwnerIsolation(t *testing.T) {
	...
	t.Run("mine session is transitive by default", func(t *testing.T) {
		t.Setenv("PRETTY_SESSION_ID", rootSession)
		t.Setenv("PRETTY_OWNER_ID", "")
	...
	t.Run("owner environment wins over session", func(t *testing.T) {
		t.Setenv("PRETTY_SESSION_ID", rootSession)
		t.Setenv("PRETTY_OWNER_ID", "alpha")
