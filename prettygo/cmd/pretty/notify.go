...
func (a *app) cmdNotify(args []string) error {
	if len(args) < 1 || len(args) > 2 {
		return fail(1, "usage: pretty notify <status|on|off> [done|waiting|lost]")
	}
	action := args[0]
	kind := ""
	if len(args) == 2 {
...
func (a *app) requestNotify(method string, body any, action string) (notifyStatus, error) {
	...
	if err != nil {
		return notifyStatus{}, fail(2, "cannot reach prettyd: %s. Start it with `pretty install`, then retry `pretty notify %s`.", err, action)
	}
	...
	if err := json.Unmarshal(response.body, &current); err != nil {
		return notifyStatus{}, fail(2, "prettyd returned an invalid notification status: %s", err)
	}
	return current, nil
}
...
func (a *app) printNotifyStatus(current notifyStatus, selected string) error {
	...
	_, err := io.WriteString(a.stdout, "Push subscription: none. Subscribe in the Pretty web UI to opt in.\n")
	return err
}
