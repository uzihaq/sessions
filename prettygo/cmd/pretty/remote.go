...
const (
	...
	walkthroughBaseURL   = "https://pretty-pty.somewhere.site/"
)
...
func (a *app) cmdRemote(args []string) error {
	if len(args) != 1 {
		return fail(1, "usage: pretty remote <enable|disable|status>")
	}
	switch args[0] {
	case "enable":
...
func (a *app) remoteEnable() error {
	...
	if err := command.Run(); err != nil {
		return fail(2, "could not enable Tailscale Serve. If a one-time HTTPS approval URL appears above, open it and retry `pretty remote enable`.")
	}
...
func (a *app) daemonListen(status tailscaleStatus) (string, int, error) {
	...
	for _, candidate := range candidates {
		...
		response.Body.Close()
		cancel()
		if response.StatusCode != 200 || decodeErr != nil || !health.OK || health.Name != "prettyd" {
			continue
		}
	...
	return "", 0, fail(2, "cannot reach prettyd at %s. Start it first with `pretty install`.", strings.Join(parts, ", "))
}
