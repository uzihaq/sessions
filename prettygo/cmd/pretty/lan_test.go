...
func TestLANCLILifecycle(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if test.name == "enable" {
				for _, want := range []string{"When asked for a token, run: pretty token", "LAN access works only on this network. From anywhere: pretty remote enable"} {
...
func TestLANStatusReportsChangedInterface(t *testing.T) {
	...
	if code != 2 || !strings.Contains(stderr.String(), "the current LAN IP no longer matches the listener") || !strings.Contains(stderr.String(), "pretty lan enable") {
		t.Fatalf("exit=%d stdout=%q stderr=%q", code, stdout.String(), stderr.String())
	}
}
...
func TestLANCLIUnreachableDaemonTeachesFix(t *testing.T) {
	...
	if code != 2 || !strings.Contains(stderr.String(), "Start it first with `pretty install`") || !strings.Contains(stderr.String(), "pretty lan enable") {
		t.Fatalf("exit=%d stdout=%q stderr=%q", code, stdout.String(), stderr.String())
	}
}
