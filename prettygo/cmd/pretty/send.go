...
func (a *app) cmdSend(args []string) error {
	if len(args) == 0 || args[0] == "" {
		return fail(1, "usage: pretty send <id> [--no-wait] [--timeout Ns] [--file path] <text...>")
	}
	idArg := args[0]
	args = args[1:]
	...
	if a.wantJSON {
		...
		io.WriteString(a.stderr, "pretty send: session exited or became unreachable before submission was confirmed\n")
	} else {
		fmt.Fprintf(a.stderr, "pretty send: could not confirm submission after %dms\n", timeout.Milliseconds())
		if isBlockingSnapshotState(result.SnapshotState) {
			...
			io.WriteString(a.stderr, "  sent but not confirmed — the session may still be starting; retry, or use `pretty wait` first\n")
