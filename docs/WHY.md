...
- **Native app direction — decision note 2026-07-18 (platform plan locked 2026-07-18)** — the app is the answer to three problems at once: (1) no browser "Not Secure" scare on LAN http; (2) keychain token storage + signed immutable client (closes the mutable-hosted-JS trust gap); (3) can EMBED tsnet/libtailscale so "install Tailscale" disappears as a user-visible step — bolting Tailscale into the daemon is pointless, bolting it into the client app is where it pays. ARCHITECTURE PRINCIPLE: the app is a WINDOW, the daemon stays a separate OS service — quitting the app must never kill sessions (runners→daemon→app lifetime layering is the differentiator; never collapse it for packaging aesthetics). PLAN: desktop = ONE Tauri codebase (scaffold exists in src-tauri/), macOS first (bundles binaries, installs/manages launchd daemon, menu-bar status), Windows/Linux from same codebase — GATED on daemon service backends (Linux systemd runner already roadmapped; Windows SCM backend is new work). Mobile = ANDROID FIRST (user's platform): Tauri 2 Android + Kotlin plugins for FCM push (reuses our E2E-encrypted push)/widgets, fallback thin Kotlin client; iOS = thin Swift client later (WKWebView terminal; APNs/Live Activities are the point of native). PWA rung SKIPPED (user decision). — roadmap — PROMOTE: yes when built
