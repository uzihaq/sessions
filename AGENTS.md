...
- Sessions is a native product for durable Claude Code, Codex, shell, and headless command sessions; its local runtime commands retain the `pretty` name.
- The native product bundles three Go runtime binaries: `pretty`, `prettyd`, and `runner` (`prettygo/cmd/`).
...
- `frontend/` — the shared React UI embedded in app builds by `scripts/build-app-runtime.sh` and in standalone daemon builds by `prettygo/scripts/build-binaries.sh`.
...
9. **Keep the native shell above the service.** Sessions.app may install, update, and inspect `prettyd`; quitting or updating the app must never terminate the daemon or a runner (`docs/NATIVE_APP.md`).
