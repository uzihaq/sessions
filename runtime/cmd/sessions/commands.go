...
func (a *app) cmdModel(args []string) error {
	...
	if current.Working {
		return fail(1, "session is mid-turn; try when idle (sessions wait %s)", id)
	}
	tool := toolOfSession(*current)
	if tool == "codex" {
		return fail(1, "live model switch for codex requires an app-server-backed session (coming); use /model in the Terminal view, or respin with: sessions new --tool codex --model <m>")
	}
	if tool != "claude-code" {
...
func (a *app) cmdKill(ids []string) error {
	if len(ids) == 0 {
		return fail(1, "usage: sessions kill <id> [<id>...]")
	}
	anyFailed := false
